...
export async function main() {
   ...
   const ETY_CHEF_ADDR = "0x1cb2bF1b705bd712E0708455985C5a2EAEF76593";
   const rewardTokenTicker = "ETY";
   const ETY_CHEF = new ethers.Contract(ETY_CHEF_ADDR, ETY_CHEF_ABI, App.provider);

   const startBlock = await ETY_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await ETY_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await ETY_CHEF.etyPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, ETY_CHEF, ETY_CHEF_ADDR, ETY_CHEF_ABI, rewardTokenTicker,
        "ety", null, rewardsPerWeek, "pendingEty");
