...
export async function main() {
  ...
  const OMEN_CHEF_ADDR = "0x6ad70613d14c34aa69E1604af91c39e0591a132e";
  const rewardTokenTicker = "OMEN";
  const OMEN_CHEF = new ethers.Contract(OMEN_CHEF_ADDR, OMEN_CHEF_ABI, App.provider);

  const startBlock = await OMEN_CHEF.startBlock();
  const currentBlock = await App.provider.getBlockNumber();

  const multiplier = await OMEN_CHEF.getMultiplier(currentBlock, currentBlock + 1);

  let rewardsPerWeek = 0
  if (currentBlock < startBlock) {
    _print(`Rewards start at block ${startBlock}\n`);
  } else {
    rewardsPerWeek = await OMEN_CHEF.omenPerBlock() / 1e18 *
  ...
  await loadAuguryContract(App, tokens, prices, OMEN_CHEF, OMEN_CHEF_ADDR, OMEN_CHEF_ABI, rewardTokenTicker,
    "omen", null, rewardsPerWeek, "pendingOmen", [1]);

  hideLoading();
}

async function loadAuguryContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printAuguryPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
function printAuguryPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
  totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printAuguryContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain, depositFee, withdrawFee);
  return apr;
}

function printAuguryContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
  rewardTokenTicker, stakeTokenTicker, unstaked, userStaked, pendingRewardTokens, fixedDecimals,
  claimFunction, rewardTokenPrice, chain, depositFee, withdrawFee) {
  fixedDecimals = fixedDecimals ?? 2;
  const approveAndStake = async function () {
    return auguryContract_stake(chefAbi, chefAddr, poolIndex, poolAddress, App)
  }
  const unstake = async function () {
    return chefContract_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
  }
  const claim = async function () {
    return auguryContract_claim(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction, claimFunction)
  }
  if (depositFee > 0) {
...
const auguryContract_claim = async function (chefAbi, chefAddress, poolIndex, App,
