...
export async function main() {
   ...
   const KONG_CHEF_ADDR = "0x29e806A0EA0e811827C4a3DCeA907CB76bC427dd";
   const rewardTokenTicker = "KONG";
   const KONG_CHEF = new ethers.Contract(KONG_CHEF_ADDR, KONG_CHEF_ABI, App.provider);

   const startBlock = await KONG_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await KONG_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await KONG_CHEF.kongPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, KONG_CHEF, KONG_CHEF_ADDR, KONG_CHEF_ABI, rewardTokenTicker,
        "kong", null, rewardsPerWeek, "pendingKong");
