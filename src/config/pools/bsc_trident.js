...
export async function main() {
  ...
  const TRIDENT_CHEF_ADDR = "0x4cf32518b713eC6D2e67AFeAC945c5013acBbC4E";
  const rewardTokenTicker = "TRIDENT";
  const TRIDENT_CHEF = new ethers.Contract(TRIDENT_CHEF_ADDR, TRIDENT_CHEF_ABI, App.provider);

  const startBlock = 8820000;
  const currentBlock = await App.provider.getBlockNumber();

  const multiplier = await TRIDENT_CHEF.getMultiplier(currentBlock, currentBlock + 1);

  let rewardsPerWeek = 0
  if (currentBlock < startBlock) {
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/8820000" target="_blank">${8820000}</a>\n`)
  } else {
    rewardsPerWeek = await TRIDENT_CHEF.tridentPerBlock() / 1e18
      * 604800 * multiplier / 3;
  }
  ...
  await loadTridentContract(App, tokens, prices, TRIDENT_CHEF, TRIDENT_CHEF_ADDR, TRIDENT_CHEF_ABI, rewardTokenTicker,
    "trident", null, rewardsPerWeek, "pendingTrident");

  hideLoading();
}

async function loadTridentContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getTridentPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
...
async function getTridentPoolInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
