...
export async function main() {
  ...
  const BDL_CHEF_ADDR = "0xA54D10C6666172824Da54C0d90BcdE36B6dAbd85";
  const rewardTokenTicker = "BDL";
  const BDL_CHEF = new ethers.Contract(BDL_CHEF_ADDR, BDL_CHEF_ABI, App.provider);
  ...
  await loadBundleContract(App, tokens, prices, BDL_CHEF, BDL_CHEF_ADDR, BDL_CHEF_ABI, rewardTokenTicker,
    "bundle", null, rewardsPerWeek, "pendingRewards");

  hideLoading();
}

async function loadBundleContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBundlePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
...
async function getBundlePoolInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
