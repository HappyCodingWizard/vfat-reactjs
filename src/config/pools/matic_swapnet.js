...
export async function main() {
   ...
   const rewardsPerWeek = await SWP_CHEF.sushiPerSecond() /1e18 * 604800;
    ...
    await loadSwapnetContract(App, tokens, prices, SWP_CHEF, SWP_CHEF_ADDR, SWP_CHEF_ABI, rewardTokenTicker,
        "SUSHI", null, rewardsPerWeek, "pendingSushi");

    hideLoading();
  }

async function loadSwapnetContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getSwapnetPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
...
async function getSwapnetPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {  
