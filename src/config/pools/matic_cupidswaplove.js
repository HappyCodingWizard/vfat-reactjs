...
export async function main() {
    ...
    const LOVE_CHEF_ADDR = "0xfA2302754AEB7209f0f10Ff2ca732e16D62A4D60";
    const rewardTokenTicker = "LOVE";
    const LOVE_CHEF = new ethers.Contract(LOVE_CHEF_ADDR, LOVE_CHEF_ABI, App.provider);

    const startBlock = await LOVE_CHEF.startBlock();
    const currentBlock = await App.provider.getBlockNumber();
    // const bonusMultiplier = await LOVE_CHEF.BONUS_MULTIPLIER();

    let rewardsPerWeek = 0
    if (currentBlock < startBlock) {
        _print(`Rewards start at block ${startBlock}\n`);
    } else {
        rewardsPerWeek = await LOVE_CHEF.LovePerBlock() / 1e18 *
            604800 / 2.1;
    }
    ...
    await loadMaticChefContract(App, tokens, prices, LOVE_CHEF, LOVE_CHEF_ADDR, LOVE_CHEF_ABI, rewardTokenTicker,
        "love", null, rewardsPerWeek, "pendingLove");
