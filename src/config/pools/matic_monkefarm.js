...
export async function main() {
  ...
  const MONKE_CHEF_ADDR = "0xca9174A42C3727F7aE9ee2b719983f2C54709E7A";
  const rewardTokenTicker = "MONKE";
  const MONKE_CHEF = new ethers.Contract(MONKE_CHEF_ADDR, MONKE_CHEF_ABI, App.provider);

  const startBlock = await MONKE_CHEF.startBlock();
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = await MONKE_CHEF.monkePerBlock() / 1e18
      * 604800 / 2.1;
  }
  ...
  await loadMonkeContract(App, tokens, prices, MONKE_CHEF, MONKE_CHEF_ADDR, MONKE_CHEF_ABI, rewardTokenTicker,
    "monke", null, rewardsPerWeek, "pendingMonke");

  hideLoading();
}

async function loadMonkeContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printMonkePool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
function printMonkePool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
  totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
