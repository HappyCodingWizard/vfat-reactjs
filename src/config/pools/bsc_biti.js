...
export async function main() {
  ...
  const BITI_CHEF_ADDR = "0x2Dc2d579e0e0EA9Ab2EF5CcA4e813Ca7769148cB";
  const rewardTokenTicker = "BITI";
  const BITI_CHEF = new ethers.Contract(BITI_CHEF_ADDR, BITI_CHEF_ABI, App.provider);

  const rewardsPerWeek = await BITI_CHEF.bitiPerBlock() / 1e18
    * 604800 / 3;

  const tokens = {};
  const prices = await getBscPrices();

  await loadBscBitiChefContract(App, tokens, prices, BITI_CHEF, BITI_CHEF_ADDR, BITI_CHEF_ABI, rewardTokenTicker,
    "biti", null, rewardsPerWeek, "pendingRewards");

  hideLoading();
}

async function loadBscBitiChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
