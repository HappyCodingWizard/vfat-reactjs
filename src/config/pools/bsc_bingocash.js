...
export async function main() {
  ...
  const BINGO_CHEF_ADDR = "0x97bdB4071396B7f60b65E0EB62CE212a699F4B08";
  const rewardTokenTicker = "sBGO";
  const BINGO_CHEF = new ethers.Contract(BINGO_CHEF_ADDR, BINGO_CHEF_ABI, App.provider);

  const rewardsPerWeek = await BINGO_CHEF.sharePerBlock() / 1e18
    * 604800 / 3;

  const tokens = {};
  const prices = await getBscPrices();

  await loadBscChefContract(App, tokens, prices, BINGO_CHEF, BINGO_CHEF_ADDR, BINGO_CHEF_ABI, rewardTokenTicker,
    "share", null, rewardsPerWeek, "pendingShare");
