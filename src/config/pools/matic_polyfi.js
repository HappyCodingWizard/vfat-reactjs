...
export async function main() {
  ...
  const PolyFi_CHEF_ADDR = "0xf0006A2B5dBF5EA6A82Dabd88D464Da12A8c7770";
  const rewardTokenTicker = "PolyFi";
  const PolyFi_CHEF = new ethers.Contract(PolyFi_CHEF_ADDR, PolyFi_CHEF_ABI, App.provider);

  const startBlock = await PolyFi_CHEF.startBlock();
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = await PolyFi_CHEF.polyfiPerBlock() / 1e18
      * 604800 / 2.1;
  }
  ...
  await loadMaticChefContract(App, tokens, prices, PolyFi_CHEF, PolyFi_CHEF_ADDR, PolyFi_CHEF_ABI, rewardTokenTicker,
    "polyfi", null, rewardsPerWeek, "pendingEgg");
