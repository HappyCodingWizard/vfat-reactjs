...
export const pageNetwork = function () {
...
export const changeWallet = async function () {
...
export const connectWallet = async function (callback) {
  try {
...
export const consoleInit = function (callback) {
  logger = document.getElementById('log')
  return init_wallet(callback)
}

export const _print_inline = function (message) {
  if (!logger) {
    logger = document.getElementById('log')
  }
...
export const lookUpPricesHistorical = async function (id, from, to) {
  return $.ajax({
...
export const getBlockNumberFromTimestamp = async function (timestamp) {
...
export const getSourceCode = async function (address) {
  return $.ajax({
...
export const lookUpPricesSevenDays = async function (id) {
  const to = Date.now() / 1000
  const from = to - 604800
  return await lookUpPricesHistorical(id, from, to)
}

export const getPricesSevenDaysStripped = async function (id) {
  const prices = await lookUpPricesSevenDays(id)
  return prices.prices.map(x => x[1])
}

export const getHistoricalPricesStripped = async function (id, from, to) {
  const prices = await lookUpPricesHistorical(id, from, to)
  return prices.prices.map(x => x[1])
}

export const getPrices24HoursStripped = async function (id) {
  const to = Date.now() / 1000
...
export const _print24HourPrice = async function (id, ticker) {
  _print('')
  try {
...
export const printBALRewards = async function (synthStakingPoolAddr, BALPrice, percentageOfBalancerPool) { }
...
export const get_synth_weekly_rewards = async function (synth_contract_instance) {
...
export const isRewardPeriodOver = async function (reward_contract_instance) {
  const now = Date.now() / 1000
  const periodFinish = await getPeriodFinishForReward(reward_contract_instance)
  return periodFinish < now
}

export const getPeriodFinishForReward = async function (reward_contract_instance) {
...
export const rewardsContract_resetApprove = async function (stakeTokenAddr, rewardPoolAddr, App) {
...
export const dao_deposit = async (App, DAO, DOLLAR) => {
  const signer = App.provider.getSigner();
...
export const rewardsContract_stake = async function (stakeTokenAddr, rewardPoolAddr, App, maxAllowance) {
  const signer = App.provider.getSigner()
...
export const rewardsContract_unstake = async function (rewardPoolAddr, App) {
  const signer = App.provider.getSigner()
...
export const rewardsContract_movetoboardroom = async function (rewardPoolAddr, App) {
  const signer = App.provider.getSigner()
...
export const rewardsContract_exit = async function (rewardPoolAddr, App) {
  const signer = App.provider.getSigner()
...
export const rewardsContract_claim = async function (rewardPoolAddr, App) {
  const signer = App.provider.getSigner()
...
export const boardroom_claim = async function (rewardPoolAddr, App) {
  const signer = App.provider.getSigner()
...
export const chefContract_stake = async function (chefAbi, chefAddress, poolIndex, stakeTokenAddr, App) {
  const signer = App.provider.getSigner()
...
export const chefContract_unstake = async function (chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
  const signer = App.provider.getSigner()
  const CHEF_CONTRACT = new ethers.Contract(chefAddress, chefAbi, signer)
...
export const chefContract_claim = async function (chefAbi, chefAddress, poolIndex, App,
...
export const chefContract_emergencyWithdraw = async function (chefAbi, chefAddress, poolIndex, App) {
  const signer = App.provider.getSigner()
...
export async function getUniPool(app, pool, poolAddress, stakingAddress) {
  const calls = [
...
export async function getGelatoPool(app, pool, poolAddress, stakingAddress) {
  const calls = [
    pool.decimals(), pool.token0(), pool.token1(), pool.symbol(), pool.name(),
    pool.totalSupply(), pool.balanceOf(stakingAddress), pool.balanceOf(app.YOUR_ADDRESS),
...
export async function getBalancerPool(app, pool, poolAddress, stakingAddress, tokens, smartToken) {
...
export async function getJar(app, jar, address, stakingAddress) {
...
export async function getCurveToken(app, curve, address, stakingAddress, minterAddress) {
...
export async function getSaddleToken(app, saddle, address, stakingAddress, swapAddress) {
...
export async function getStableswapToken(app, stable, address, stakingAddress) {
...
export async function getErc20(app, token, address, stakingAddress) {
...
export async function getDSToken(app, token, address, stakingAddress) {
  const calls = [token.decimals(), token.balanceOf(stakingAddress), token.balanceOf(app.YOUR_ADDRESS),
  token.name(), token.symbol(), token.totalSupply()];
...
export async function getVault(app, vault, address, stakingAddress) {
...
export async function getCToken(app, cToken, address, stakingAddress) {
...
export async function getStoredToken(app, tokenAddress, stakingAddress, type) {
  switch (type) {
    case "uniswap":
...
export async function getToken(app, tokenAddress, stakingAddress) {
...
export function getGelatoPrices(tokens, prices, pool, chain = "eth") {
...
export function getUniPrices(tokens, prices, pool, chain = "eth") {
  var t0 = getParameterCaseInsensitive(tokens, pool.token0);
  var p0 = getParameterCaseInsensitive(prices, pool.token0)?.usd;
  var t1 = getParameterCaseInsensitive(tokens, pool.token1);
  var p1 = getParameterCaseInsensitive(prices, pool.token1)?.usd;
...
export function getValuePrices(tokens, prices, pool) {
  var t0 = getParameterCaseInsensitive(tokens, pool.token0);
  var p0 = getParameterCaseInsensitive(prices, pool.token0)?.usd;
  var t1 = getParameterCaseInsensitive(tokens, pool.token1);
  var p1 = getParameterCaseInsensitive(prices, pool.token1)?.usd;
...
export function getBalancerPrices(tokens, prices, pool) {
  ...
  if (missing.length === poolPrices.length) {
    const msg = 'Every price is missing';
    throw msg;
  }
...
export function getBunicornPrices(tokens, prices, pool) {
...
export function getWrapPrices(tokens, prices, pool) {
...
export function getErc20Prices(prices, pool, chain = "eth") {
...
export function getCurvePrices(prices, pool) {
...
export async function getPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction, showAll = false) {
...
export function printAPR(rewardTokenTicker, rewardPrice, poolRewardsPerWeek,
...
export function printChefContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
...
export async function loadSingleChefPool(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
...
export async function loadBoardroom(App, prices, boardroomAddress, oracleAddress, lptAddress, rewardTokenAddress, stakeTicker, rewardTicker,
...
export async function loadSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
export async function loadBasisFork(data) {
...
export async function getNewPricesAndTokens(App, tokens, prices, newAddresses, stakingAddress) {
...
export async function getAverageBlockTime(App) {
