...
export async function main() {
   ...
   const PAPAS_CHEF_ADDR = "0xaEB912256F50B23BA865AAF865434F7eCe7618bc";
   const rewardTokenTicker = "PAPAS";
   const PAPAS_CHEF = new ethers.Contract(PAPAS_CHEF_ADDR, PAPAS_CHEF_ABI, App.provider);

   const startBlock = await PAPAS_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await PAPAS_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await PAPAS_CHEF.PapasPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, PAPAS_CHEF, PAPAS_CHEF_ADDR, PAPAS_CHEF_ABI, rewardTokenTicker,
        "PAPAS", null, rewardsPerWeek, "pendingPapas");
