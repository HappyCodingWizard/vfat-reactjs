...
export async function main() {
   ...
   const SMRF_CHEF_ADDR = "0x8045CFc4B0551e6186Bad0F8E5Cc532Ac44e214C";
   const rewardTokenTicker = "SMRF";
   const SMRF_CHEF = new ethers.Contract(SMRF_CHEF_ADDR, SMRF_CHEF_ABI, App.provider);

   const startBlock = await SMRF_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await SMRF_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await SMRF_CHEF.smrfPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, SMRF_CHEF, SMRF_CHEF_ADDR, SMRF_CHEF_ABI, rewardTokenTicker,
        "smrf", null, rewardsPerWeek, "pendingSmrf");
