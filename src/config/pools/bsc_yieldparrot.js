...
export async function main() {
  ...
  const LORY_CHEF_ADDR = "0x1bee93b82275F3F215411bE49F948F8568e5e103";
  const rewardTokenTicker = "LORY";
  const LORY_CHEF = new ethers.Contract(LORY_CHEF_ADDR, LORY_CHEF_ABI, App.provider);

  const rewardsPerWeek = await LORY_CHEF.NATIVEPerBlock() / 1e18
    * 604800 / 3;

  const tokens = {};
  const prices = await getBscPrices();

  await loadBscLORYContract(App, tokens, prices, LORY_CHEF, LORY_CHEF_ADDR, LORY_CHEF_ABI, rewardTokenTicker,
    "NATIVE", null, rewardsPerWeek, "pendingNATIVE", [1]);

  hideLoading();
}

async function loadBscLORYContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBscLORYPoolInfo(App, chefContract, x, pendingRewardsFunction)));
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printLORYPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getBscLORYPoolInfo(App, chefContract, poolIndex, pendingRewardsFunction) {
...
function printLORYPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
  totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printLORYContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printLORYContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
  ...
  const unstake = async function () {
    return chefLORYContract_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
  }
...
const chefLORYContract_unstake = async function (chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
