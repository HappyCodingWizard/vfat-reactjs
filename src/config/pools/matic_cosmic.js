...
export async function main() {
  ...
  const COSMIC_CHEF_ADDR = "0x7cEFc4449f047FaF16eeD516b80a9E49D3c45F6F";
  const rewardTokenTicker = "COSMIC";
  const COSMIC_CHEF = new ethers.Contract(COSMIC_CHEF_ADDR, COSMIC_CHEF_ABI, App.provider);

  const startBlock = await COSMIC_CHEF.startBlock();
  const currentBlock = await App.provider.getBlockNumber();

  const multiplier = await COSMIC_CHEF.getMultiplier(currentBlock, currentBlock + 1);
  ...
  await loadMaticChefContract(App, tokens, prices, COSMIC_CHEF, COSMIC_CHEF_ADDR, COSMIC_CHEF_ABI, rewardTokenTicker,
    "cosmic", null, rewardsPerWeek, "pendingCosmic");

  hideLoading();
}

async function getCosmicPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  const poolInfo = await chefContract.poolInfo(poolIndex);
  if (poolInfo.allocPoint === 0 || poolInfo.lpToken === "0x000000000000000000000000000000000000dEaD") {
    return {
...
async function loadMaticChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getCosmicPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
