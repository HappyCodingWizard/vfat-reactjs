...
const NdxContracts = [
    { name: "NDXUNIDEGENPool", address: "0x649A94e2b3010338508CF50865BaafB1FA07A32c" },
    { name: "NDXUNIORCL5Pool", address: "0x840AdeCd76e81829F3ef083062b3fB9719243E9F" },
...
export async function main() {
    ...
    await loadIndexedChefContract(App, NDX_CHEF, NDX_CHEF_ADDR, NDX_CHEF_ABI,
        "NDX", "rewardsToken", null, rewardsPerWeek, "pendingRewards");
...
async function loadIndexedChefContract(App, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) => {
    try {
      return await getIndexedPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction, showAll);
    }
    catch (ex) {
...
async function getIndexedPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction, showAll=false) {
