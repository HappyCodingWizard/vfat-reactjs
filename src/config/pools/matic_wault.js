...
export async function main() {
  ...
  const WEXpoly_CHEF_ADDR = "0xC8Bd86E5a132Ac0bf10134e270De06A8Ba317BFe";
  const rewardTokenTicker = "WEXpoly";
  const WEXpoly_CHEF = new ethers.Contract(WEXpoly_CHEF_ADDR, WEXpoly_CHEF_ABI, App.provider);

  const startBlock = await WEXpoly_CHEF.startBlock();
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = await WEXpoly_CHEF.wexPerBlock() / 1e18
      * 604800 / 2.1;
  }
  ...
  await loadMaticChefContract(App, tokens, prices, WEXpoly_CHEF, WEXpoly_CHEF_ADDR, WEXpoly_CHEF_ABI, rewardTokenTicker,
    "wex", null, rewardsPerWeek, "pendingWex");
