...
export async function main() {
    const params = Contracts.BSC_SPAD.Parameters;
    const App = await init_ethers();

    _print(`Initialized ${App.YOUR_ADDRESS}\n`);
    _print("Reading smart contracts...\n");

    const DAO = new ethers.Contract(Contracts.BSC_SPAD.DAO.address,
        Contracts.BSC_SPAD.DAO.abi, App.provider);
    const DOLLAR = new ethers.Contract(Contracts.BSC_SPAD.Dollar.address,
        ERC20_ABI, App.provider);
    var prices = { "0x8ac76a51cc950d9822d68b83fe1ad97b32cd580d": { usd: 1 } }; //USDC
    var tokens = {};

    const [epoch, uniPrices, totalBonded] = await loadDAO(App, DAO, DOLLAR, Contracts.BSC_SPAD.UniswapLP.address,
        Contracts.BSC_SPAD.LPIncentivizationPool.address, tokens, prices, params.DaoLockupPeriods);

    const LP = new ethers.Contract(Contracts.BSC_SPAD.LPIncentivizationPool.address,
        Contracts.BSC_SPAD.LPIncentivizationPool.abi, App.provider);
    await loadEmptySetLP(App, LP, Contracts.BSC_SPAD.UniswapLP.address,
    ...
    if (epoch < params.BootstrappingPeriod) { //bootstrapping
        ...
        const resp = await fetch('https://api.vfat.tools/twap/' + Contracts.BSC_SPAD.UniswapLP.address);
        const text = await resp.text();
        const array = text.split("\n");
        if (array.length > 0) {
            const [, oldPrice1, oldTimestamp] = array[array.length - 2].split(' ') //last line is blank
            const [, price1, timestamp] = await getCurrentPriceAndTimestamp(App, Contracts.BSC_SPAD.UniswapLP.address);
