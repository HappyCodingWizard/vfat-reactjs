...
export async function main() {
  ...
  const FRANK_CHEF_ADDR = "0xA07fcB5eDf05EA16e681ca7019e696c7DaD2ee4a";
  const rewardTokenTicker = "FRANK";
  const FRANK_CHEF = new ethers.Contract(FRANK_CHEF_ADDR, FRANK_CHEF_ABI, App.provider);
  ...
  await loadBscFRANKContract(App, tokens, prices, FRANK_CHEF, FRANK_CHEF_ADDR, FRANK_CHEF_ABI, rewardTokenTicker,
    "NATIVE", null, rewardsPerWeek, "pendingNATIVE", [3, 6]);

  hideLoading();
}

async function loadBscFRANKContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBscFRANKPoolInfo(App, chefContract, x, pendingRewardsFunction)));
...
async function getBscFRANKPoolInfo(App, chefContract, poolIndex, pendingRewardsFunction) {
