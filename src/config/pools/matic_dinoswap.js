...
export async function main() {
   ...
   const DINO_CHEF_ADDR = "0x1948abC5400Aa1d72223882958Da3bec643fb4E5";
   const rewardTokenTicker = "DINO";
   const DINO_CHEF = new ethers.Contract(DINO_CHEF_ADDR, DINO_CHEF_ABI, App.provider);

   const rewardsPerWeek = await DINO_CHEF.dinoPerBlock() /1e18
      * 604800 / 2.1;

    const tokens = {};
    const prices = await getMaticPrices();

    await loadMaticChefContract(App, tokens, prices, DINO_CHEF, DINO_CHEF_ADDR, DINO_CHEF_ABI, rewardTokenTicker,
        "DINO", null, rewardsPerWeek, "pendingDino");
