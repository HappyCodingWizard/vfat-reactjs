...
const DEER_CHEF_ABI = [
  {
    "inputs": [
      {
        "internalType": "contract GemstoneToken",
        "name": "_gemstones",
        "type": "address"
      },
      {
...
export async function main() {
  ...
  const DEER_CHEF_ADDR = "0x9BFD897e3eabFfA738a8F1c4d0B397C07E97E42D";
  const rewardTokenTicker = "GMS";
  const DEER_CHEF = new ethers.Contract(DEER_CHEF_ADDR, DEER_CHEF_ABI, App.provider);

  const startBlock = await DEER_CHEF.startBlock();
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = await DEER_CHEF.gemstonesPerBlock() / 1e18
      * 604800 / 2.1;
  }
  ...
  await loadMaticChefContract(App, tokens, prices, DEER_CHEF, DEER_CHEF_ADDR, DEER_CHEF_ABI, rewardTokenTicker,
    "gemstones", null, rewardsPerWeek, "pendingGemstones");
