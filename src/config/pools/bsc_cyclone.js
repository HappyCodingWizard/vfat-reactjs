...
export async function main() {
  ...
  const CYC_CHEF_ADDR = "0x567da514637CfD7F9e1f185ae4aa163b3ebB5363";
  ...
  await loadCycloneChefContract(App, tokens, prices, CYC_CHEF, CYC_CHEF_ADDR, CYC_CHEF_ABI, rewardTokenTicker,
    "cycToken", null, rewardsPerWeek, "pendingReward");

  hideLoading();
}

async function loadCycloneChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getCyclonePoolInfo(App, chefContract, chefAddress, pendingRewardsFunction)));
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printCycloneChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getCyclonePoolInfo(App, chefContract, chefAddress, pendingRewardsFunction) {
...
function printCycloneChefPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
  rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printCycloneContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printCycloneContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
  rewardTokenTicker, stakeTokenTicker, unstaked, userStaked, pendingRewardTokens, fixedDecimals,
  claimFunction, rewardTokenPrice) {
  fixedDecimals = fixedDecimals ?? 2;
  const approveAndStake = async function () {
    return cycloneContract_stake(chefAbi, chefAddr, poolAddress, App)
  }
  const unstake = async function () {
    return cycloneContract_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
  }
...
const cycloneContract_stake = async function (chefAbi, chefAddress, stakeTokenAddr, App) {
...
const cycloneContract_unstake = async function (chefAbi, chefAddress, App, pendingRewardsFunction) {
  const signer = App.provider.getSigner()
  const CHEF_CONTRACT = new ethers.Contract(chefAddress, chefAbi, signer)
