...
export async function main() {
  ...
  const LIME_CHEF_ADDR = "0x56dB5D1A7Ac52B55131EE71762b6Bd7829716BF4";
  const rewardTokenTicker = "LIME";
  const LIME_CHEF = new ethers.Contract(LIME_CHEF_ADDR, LIME_CHEF_ABI, App.provider);

  const startBlock = await LIME_CHEF.startBlock();
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = await LIME_CHEF.limePerBlock() / 1e18
      * 604800 / 3;
  }
  ...
  await loadBscChefContract(App, tokens, prices, LIME_CHEF, LIME_CHEF_ADDR, LIME_CHEF_ABI, rewardTokenTicker,
    "lime", null, rewardsPerWeek, "pendingLime");
