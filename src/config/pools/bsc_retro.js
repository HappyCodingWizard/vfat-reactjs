...
export async function main() {
  ...
  const QBert_CHEF_ADDR = "0x738600B15B2b6845d7Fe5B6C7Cb911332Fb89949";
  const rewardTokenTicker = "QBert";
  const QBert_CHEF = new ethers.Contract(QBert_CHEF_ADDR, QBert_CHEF_ABI, App.provider);

  const startBlock = await QBert_CHEF.startBlock();
  const currentBlock = await App.provider.getBlockNumber();

  const multiplier = await QBert_CHEF.getMultiplier(currentBlock, currentBlock + 1);

  let rewardsPerWeek = 0
  if (currentBlock < startBlock) {
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
  } else {
    rewardsPerWeek = await QBert_CHEF.NATIVEPerBlock() / 1e18
      * 604800 * multiplier / 3;
  }
  ...
  await loadBscQBertContract(App, tokens, prices, QBert_CHEF, QBert_CHEF_ADDR, QBert_CHEF_ABI, rewardTokenTicker,
    "NATIVE", null, rewardsPerWeek, "pendingNATIVE", [5, 6]);

  hideLoading();
}

async function loadBscQBertContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBscQBertPoolInfo(App, chefContract, x, pendingRewardsFunction)));
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printQBertPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getBscQBertPoolInfo(App, chefContract, poolIndex, pendingRewardsFunction) {
...
function printQBertPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
  totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printQBertContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printQBertContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
  ...
  const unstake = async function () {
    return chefQBertContract_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
  }
...
const chefQBertContract_unstake = async function (chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
