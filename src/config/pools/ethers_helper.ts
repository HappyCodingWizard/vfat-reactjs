...
export const networkNameFromId = function (id: any) {
  let network: any;
  for (network of Object.values(NETWORKS)) {
...
const init_wallet = async function (callback: any) {
...
export async function init_ethers() {
  ...
  try {
    ...
    let addr: any = getUrlParameter("addr");
...
const switchNetwork = async function (network: any) {
...
export const connectWallet = async function (callback: any) {
  try {
    ...
    walletProvider.on("chainChanged", (networkId: any) => {
      window.location.reload();
    });
    ...
    if (connectedNetwork.chainId === targetNetworkId) {
      $("#connect_wallet_button > span").text("Change Wallet");
      ...
      let button: any = document.getElementById("connect_wallet_button");
      $(document).off("click", "#connect_wallet_button");
      button.remove();
...
export const getUrlParameter = function (sParam: any) {
...
export const toFixed = function (num: any, fixed: any) {
...
export const consoleInit = function (callback: any) {
  logger = document.getElementById("log");
  return init_wallet(callback);
};

export const _print_inline = function (message: any) {
  if (!logger) {
    logger = document.getElementById("log");
  }
...
export const _print_link = function (
  message: any,
  onclickFunction: any,
  uuid: any = ID()
) {
  ...
  switch (uuid) {
    case "connect_wallet_button":
    case "clear_browser_storage":
...
const _print_href = function (message: any, href: any) {
  if (!logger) {
    logger = document.getElementById("log");
  }
...
const sleep = function (milliseconds: any) {
  const date = Date.now();
...
export const chunk = (arr: any, n: any): any =>
  arr.length ? [arr.slice(0, n), ...chunk(arr.slice(n), n)] : [];

export const lookUpPrices = async function (id_array: any) {
  const prices: any = {};
  for (const id_chunk of chunk(id_array, 50)) {
    ...
    for (const [key, v] of Object.entries(res)) {
      if (Object(v).usd) prices[key] = v;
    }
  }
  return prices;
};

export const lookUpTokenPrices = async function (id_array: any) {
  const prices: any = [];
  for (const id_chunk of chunk(id_array, 50)) {
    let ids = id_chunk.join("%2C");
    let res = await $.ajax({
...
export const lookUpPricesHistorical = async function (id: any, from: any, to: any) {
  return $.ajax({
...
export const getBlockNumberFromTimestamp = async function (timestamp: any) {
...
export const getSourceCode = async function (address: any) {
  return $.ajax({
...
export const lookUpPricesSevenDays = async function (id: any) {
  const to = Date.now() / 1000;
  const from = to - 604800;
  return await lookUpPricesHistorical(id, from, to);
};

const getPricesSevenDaysStripped = async function (id: any) {
  const prices = await lookUpPricesSevenDays(id);
  return prices.prices.map((x: any) => x[1]);
};

const getHistoricalPricesStripped = async function (
  id: any,
  from: any,
  to: any
) {
...
const getPrices24HoursStripped = async function (id: any) {
  const to = Date.now() / 1000;
...
export const _print24HourPrice = async function (id: any, ticker: any) {
  _print("");
  try {
...
export const getBlockTime = function () {
  ...
  return new Promise((resolve: any, reject: any) => {
    $.ajax({
      ...
      type: "GET",
      success: function (data: any, text: any) {
...
const printBALRewards = async function (
  synthStakingPoolAddr: any,
  BALPrice: any,
  percentageOfBalancerPool: any
) {};
...
const safeParseFloat = function (str: any) {
  let res = parseFloat(str);
  return res ? res : 0;
};

// export const getBALEarnings = async function (addr: any, startWeek: any) {
//   // SNX-usdc Redirect
...
export const get_synth_weekly_rewards = async function (synth_contract_instance: any) {
...
export const isRewardPeriodOver = async function (reward_contract_instance: any) {
...
export function sleep_async(ms: any) {
 ...
 */
export const forHumans = function (seconds: any) {
  const levels: any = [
...
export const rewardsContract_resetApprove = async function (
  stakeTokenAddr: any,
  rewardPoolAddr: any,
  App: any
) {
...
const trimOrFillTo = function (str: any, n: any) {
  str = str + "";
...
export const rewardsContract_stake = async function (
  stakeTokenAddr: any,
  rewardPoolAddr: any,
  App: any,
  maxAllowance: any = undefined
) {
  const signer = App.provider.getSigner();
  ...
  const currentTEND = maxAllowance
    ? maxAllowance / 1e18 < balanceOf / 1e18
      ? maxAllowance
      : balanceOf
    : balanceOf;
...
export const rewardsContract_unstake = async function (rewardPoolAddr: any, App: any) {
  const signer = App.provider.getSigner();
...
export const rewardsContract_movetoboardroom = async function (
  rewardPoolAddr: any,
  App: any
) {
  const signer = App.provider.getSigner();
...
export const rewardsContract_exit = async function (rewardPoolAddr: any, App: any) {
  const signer = App.provider.getSigner();
...
export const rewardsContract_claim = async function (rewardPoolAddr: any, App: any) {
  const signer = App.provider.getSigner();
...
export const boardroom_claim = async function (rewardPoolAddr: any, App: any) {
  const signer = App.provider.getSigner();
...
export const print_warning = function () {
  _print_bold(
    "WARNING: DO NOT USE STAKE IN THIS POOL UNLESS YOU HAVE REVIEWED THE CONTRACTS."
  );
  _print_bold(
    "         YOU ARE RESPONSIBLE FOR ANY FUNDS THAT YOU LOSE BY INTERACTING WITH THIS CONTRACT.\n"
  );
};

export const chefContract_stake = async function (
  chefAbi: any,
  chefAddress: any,
  poolIndex: any,
  stakeTokenAddr: any,
...
export const chefContract_unstake = async function (
  chefAbi: any,
  chefAddress: any,
  poolIndex: any,
  App: any,
...
export const chefContract_claim = async function (
  chefAbi: any,
  chefAddress: any,
  poolIndex: any,
  App: any,
  ...
  if (earnedTokenAmount > 0) {
    showLoading();
    if (claimFunction) {
      claimFunction(poolIndex, { gasLimit: 500000 }).then(function (t: any) {
        return App.provider.waitForTransaction(t.hash);
      });
    } else {
...
export const chefContract_emergencyWithdraw = async function (
  chefAbi: any,
  chefAddress: any,
  poolIndex: any,
  App: any
...
export async function getUniPool(
  app: any,
  pool: any,
  poolAddress: any,
  stakingAddress: any
...
export async function getGelatoPool(
  app: any,
  pool: any,
  poolAddress: any,
  stakingAddress: any
...
export async function getBalancerPool(
  ...
  stakingAddress: any,
  tokens: any,
  smartToken: any = undefined
) {
  const tokenCalls = tokens
    .map((t: any) => [pool.getNormalizedWeight(t), pool.getBalance(t)])
    .flat();
  const calls = [
    pool.decimals(),
    pool.symbol(),
  ...
  if (smartToken) {
    [totalSupply, staked, unstaked] = await app.ethcallProvider.all([
...
async function getSaddleToken(
  ...
  const calls = [
    saddle.decimals(),
    saddle.balanceOf(stakingAddress),
    saddle.balanceOf(app.YOUR_ADDRESS),
    saddle.name(),
    saddle.symbol(),
    saddle.totalSupply(),
  ];
...
async function getStableswapToken(
  app: any,
  stable: any,
  address: any,
  stakingAddress: any
...
async function getStoredToken(
  ...
  switch (type) {
    ...
    case "balancerSmart":
      ...
      const [tokens2] = await app.ethcallProvider.all([
        bal2.getCurrentTokens(),
      ]);
      return await getBalancerPool(
        app,
        bal2,
    ...
    case "curve":
      ...
      if (
        ...
        "0x88E11412BB21d137C217fd8b73982Dc0ED3665d7".toLowerCase()
      ) {
        const minter = "0x3333333ACdEdBbC9Ad7bda0876e60714195681c5";
        return await getCurveToken(
          app,
          crv,
          tokenAddress,
          stakingAddress,
    ...
    case "stableswap":
      ...
      return await getStableswapToken(
        app,
        stable,
        tokenAddress,
        stakingAddress
...
export async function getToken(app: any, tokenAddress: any, stakingAddress: any) {
  ...
  try {
    ...
    const gelatoPool = await getGelatoPool(
      app,
      gelato,
      tokenAddress,
      stakingAddress
  ...
  try {
    const sbal = new ethcall.Contract(tokenAddress, BPOOL_TOKEN_ABI);
    const [bPool] = await app.ethcallProvider.all([
      sbal.bPool(),
      sbal.totalSupply(),
    ]);
  ...
  try {
    const saddle = new ethcall.Contract(tokenAddress, SADDLE_LP_TOKEN_ABI);
    const [swap] = await app.ethcallProvider.all([saddle.swap()]);
    const res = await getSaddleToken(
      app,
      saddle,
      tokenAddress,
      stakingAddress,
...
function getGelatoPrices(
  ...
  return {
    ...
    print_price(chain = "eth") {
      ...
      _print(
        `<a href='${poolUrl}' target='_blank'>${stakeTokenTicker}</a> Price: $${formatMoney(
          price
        )} TVL: $${formatMoney(tvl)}`
      );
      ...
      _print(
        `Staked: ${pool.staked.toFixed(4)} ${pool.symbol} ($${formatMoney(
          staked_tvl
        )})`
      );
    },
    print_contained_price(userStaked: any) {
...
export function getUniPrices(tokens: any, prices: any, pool: any, chain: any = "eth") {
  var t0 = getParameterCaseInsensitive(tokens, pool.token0);
  var p0 = getParameterCaseInsensitive(prices, pool.token0)?.usd;
  var t1 = getParameterCaseInsensitive(tokens, pool.token1);
  var p1 = getParameterCaseInsensitive(prices, pool.token1)?.usd;
  ...
  else if (pool.symbol.includes("OperaSwap"))
    stakeTokenTicker += " Opera Swap LP";
  ...
  else if (pool.symbol.includes("Field-LP"))
    stakeTokenTicker += " Yield Fields LP";
  ...
  return {
    ...
    staked_tvl: staked_tvl,
    stakeTokenTicker: stakeTokenTicker,
    print_price(chain: string = "eth", decimals: any, customURLs: any) {
      const t0address = t0.symbol === "ETH" ? "ETH" : t0.address;
      const t1address = t1.symbol === "ETH" ? "ETH" : t1.address;
      if (customURLs) {
        ...
        _print(`${t1.symbol} Price: $${displayPrice(p1)}`);
        _print(
          `Staked: ${pool.staked.toFixed(decimals ?? 4)} ${
            pool.symbol
        ...
        const poolUrl = pool.is1inch
          ...
          ? `https://info.apeswap.finance/pair/${pool.address}`
          : pool.symbol.includes("ZDEXLP")
          ? `https://charts.zoocoin.cash/?exchange=ZooDex&pair=${t0.symbol}-${t1.symbol}`
        ...
        const helperUrls: any = pool.is1inch
          ? []
          : pool.symbol.includes("LSLP")
          ? [
    ...
    pair_links(chain = "eth", decimals: any, customURLs: any) {
      const t0address = t0.symbol === "ETH" ? "ETH" : t0.address;
      const t1address = t1.symbol === "ETH" ? "ETH" : t1.address;
      if (customURLs) {
        const poolUrl = `${customURLs.info}/${pool.address}`;
...
export function getValuePrices(tokens: any, prices: any, pool: any) {
  var t0 = getParameterCaseInsensitive(tokens, pool.token0);
  var p0 = getParameterCaseInsensitive(prices, pool.token0)?.usd;
  var t1 = getParameterCaseInsensitive(tokens, pool.token1);
  var p1 = getParameterCaseInsensitive(prices, pool.token1)?.usd;
...
export function getBalancerPrices(tokens: any, prices: any, pool: any) {
  var poolTokens = pool.poolTokens.map((t: any) =>
    getParameterCaseInsensitive(tokens, t.address)
  );
  var poolPrices = pool.poolTokens.map(
    (t: any) => getParameterCaseInsensitive(prices, t.address)?.usd
  );
  var quantities = poolTokens.map(
    (t: any, i: any) => pool.poolTokens[i].balance / 10 ** t.decimals
  );
  var missing = poolPrices
    .map((x: any, i: any) => (x ? -1 : i))
    .filter((x: any) => x >= 0);
  ...
  var tvl = poolPrices
    .map((p: any, i: any) => p * quantities[i])
    .reduce((x: any, y: any) => x + y, 0);
  var price = tvl / pool.totalSupply;
  prices[pool.address] = { usd: price };
  var staked_tvl = pool.staked * price;
  var tickers = pool.poolTokens.map(
    (pt: any, i: any) => `[${poolTokens[i].symbol} ${pt.weight * 100}%]`
  );
  ...
  return {
    ...
    print_price() {
      ...
      poolPrices.forEach((p: any, i: any) =>
        _print(`${poolTokens[i].symbol} Price: $${formatMoney(p)}`)
      );
      _print(
        `Staked: ${pool.staked.toFixed(4)} ${stakeTokenTicker} ($${formatMoney(
          staked_tvl
        )})`
      );
    },
    print_contained_price(userStaked: any) {
      ...
      var userQs = quantities.map(
        (q: any, i: any) =>
          `${(q * userPct).toFixed(4)} ${poolTokens[i].symbol}`
      );
...
export function getWrapPrices(tokens: any, prices: any, pool: any) {
  const wrappedToken = pool.token;
  if (wrappedToken.token0 != null) {
    //Uniswap
    const uniPrices: any = getUniPrices(tokens, prices, wrappedToken);
    const poolUrl = pool.is1inch
      ? "https://1inch.exchange/#/dao/pools"
      : pool.symbol.includes("SLP")
      ? `http://analytics.sushi.com/pairs/${wrappedToken.address}`
      : pool.symbol.includes("Cake") || pool.symbol.includes("Pancake")
    ...
    const price =
      ((pool.balance / 10 ** wrappedToken.decimals) * uniPrices.price) /
    ...
    return {
      ...
      print_price() {
        _print(
          `${name} Price: $${formatMoney(price)} TVL: $${formatMoney(tvl)}`
        );
        _print(
          `Staked: ${pool.staked.toFixed(4)} ${pool.symbol} ($${formatMoney(
            staked_tvl
      ...
      print_contained_price(_: any) {},
    };
  } else {
    let tokenPrice = 0;
    if (wrappedToken.token != null) {
      //e.g. stakedao crv token vault
      const pp: any = getPoolPrices(tokens, prices, wrappedToken.token);
      tokenPrice = pp.price;
    } else {
    ...
    const price =
      ((pool.balance / 10 ** wrappedToken.decimals) * tokenPrice) /
      (pool.totalSupply / 10 ** pool.decimals);
    const tvl = (pool.balance / 10 ** wrappedToken.decimals) * price;
    const staked_tvl = pool.staked * price;
    prices[pool.address] = { usd: price };
    return {
      ...
      print_price() {
        _print(
          `${pool.symbol} Price: $${formatMoney(price)} TVL: $${formatMoney(
            tvl
          )}`
        );
        _print(
...
export function getErc20Prices(prices: any, pool: any, chain: any = "eth") {
  ...
  return {
    ...
    print_price() {
      _print(
        `${name} Price: $${displayPrice(price)} Market Cap: $${formatMoney(
          tvl
        )} ${getDexguruTokenlink()}`
      );
      _print(
...
export function getCurvePrices(prices: any, pool: any) {
  var price =
    (getParameterCaseInsensitive(prices, pool.token.address)?.usd ?? 1) *
    pool.virtualPrice;
  if (getParameterCaseInsensitive(prices, pool.address)?.usd ?? true) {
    prices[pool.address] = { usd: price };
  }
  var tvl = (pool.totalSupply * price) / 10 ** pool.decimals;
  var staked_tvl = pool.staked * price;
  ...
  return {
    ...
    print_price() {
      _print(
        `${name} Price: $${formatMoney(price)} Market Cap: $${formatMoney(
          tvl
        )} ${getDexguruTokenlink()}`
      );
      _print(
...
export async function getPoolInfo(
  ...
  const poolToken = await getToken(
    app,
    poolInfo.lpToken ?? poolInfo.stakingToken,
    chefAddress
  );
...
export function printAPR(
  rewardTokenTicker: any,
  rewardPrice: any,
  poolRewardsPerWeek: any,
  stakeTokenTicker: any,
  staked_tvl: any,
  userStaked: any,
  poolTokenPrice: any,
  fixedDecimals: any
...
export function printChefContractLinks(
  App: any,
  chefAbi: any,
  chefAddr: any,
  poolIndex: any,
  ...
  unstaked: any,
  userStaked: any,
  pendingRewardTokens: any,
  fixedDecimals: any,
  claimFunction: any,
  rewardTokenPrice: any,
  chain: any,
  depositFee: any,
  withdrawFee: any
) {
...
export function printChefPool(
  ...
  pendingRewardsFunction: any,
  fixedDecimals: any = undefined,
  claimFunction: any = undefined,
  chain: any = "eth",
  depositFee: any = 0,
  withdrawFee: any = 0
) {
...
export async function loadChefContract(
  App: any,
  chef: any,
  chefAddress: any,
  chefAbi: any,
  ...
  _print(
    `<a href='https://etherscan.io/address/${chefAddress}' target='_blank'>Staking Contract</a>`
  );
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(async (x: any) => {
      try {
  ...
  await Promise.all(
    tokenAddresses.map(async (address: number) => {
  ...
  if (deathPoolIndices) {
    ...
    deathPoolIndices
      .map((i: any) => poolInfos[i])
      .map((poolInfo: any) =>
        poolInfo.poolToken
          ? getPoolPrices(tokens, prices, poolInfo.poolToken, "eth")
          : undefined
      );
  }
...
export async function loadSingleChefPool(
  App: any,
  tokens: any,
  prices: any,
  chef: any,
  ...
  await getNewPricesAndTokens(
    App,
    tokens,
    prices,
    poolInfo.poolToken.tokens.concat([rewardTokenAddress]),
    chefAddress
  );
...
export async function loadBoardroom(
  App: any,
  prices: any,
  boardroomAddress: any,
  oracleAddress: any,
  lptAddress: any,
  rewardTokenAddress: any,
  stakeTicker: any,
  rewardTicker: any,
  epochsPerDay: any,
  maxSupplyIncrease: any,
  decimals: any,
  ratio: any,
  targetMantissa: any
) {
  const BOARDROOM = new ethers.Contract(
    boardroomAddress,
    BOARDROOM_ABI,
    App.provider
  );
  ...
  _print(`Boardroom`);
  _print(
    `There is a total ${totalStaked.toFixed(2)} ${stakeTicker} ($${formatMoney(
      totalStakedUsd
    )}) staked in the Boardroom.`
  );
  _print(
    `You are staking ${userStaked} ${stakeTicker} ($${formatMoney(
      userStakedUsd
    )}), ${userPct.toFixed(2)}% of the pool.`
  );
  ...
  if (token0.toLowerCase() === rewardTokenAddress.toLowerCase()) {
    ...
    const [price0, , timestamp] = await getCurrentPriceAndTimestamp(
      App,
      lptAddress
    );
    ...
    const [, price1, timestamp] = await getCurrentPriceAndTimestamp(
      App,
      lptAddress
    );
    twap = await calculateTwap(
  ...
  if (twap > 1) {
    ...
    const totalSupply =
      (await REWARD_TOKEN.totalSupply()) /
      10 ** (await REWARD_TOKEN.decimals());
    const newTokens =
      totalSupply * Math.min(twap - 1, maxSupplyIncrease) * ratio;
    _print(
      `There will be ${newTokens.toFixed(
        decimals
      )} ${rewardTicker} issued at next expansion.`
    );
    const boardReturn =
      ((newTokens * rewardPrice) / totalStakedUsd) * 100 * epochsPerDay;
    _print(
      `Boardroom APR: Day ${boardReturn.toFixed(2)}% Week ${(
        boardReturn * 7
      ).toFixed(2)}% Year ${(boardReturn * 365).toFixed(2)}%`
    );
  }

  const approveTENDAndStake = async () =>
    rewardsContract_stake(share, boardroomAddress, App);
  ...
  const revoke = async () =>
    rewardsContract_resetApprove(share, boardroomAddress, App);

  _print_link(
    `Stake ${userUnstaked.toFixed(decimals)} ${stakeTicker}`,
    approveTENDAndStake
  );
  _print_link(
    `Unstake ${userStaked.toFixed(decimals)} ${stakeTicker}`,
    unstake
  );
  _print_link(
    `Claim ${earned.toFixed(decimals)} ${rewardTicker} ($${formatMoney(
      earned * rewardPrice
    )})`,
    claim
  );
...
export async function loadSynthetixPoolInfo(
  App: any,
  tokens: any,
  prices: any,
  stakingAbi: any,
  ...
  var newTokenAddresses = stakeToken.tokens.filter(
    (x: any) => !getParameterCaseInsensitive(tokens, x)
  );
  ...
  const poolPrices: any = getPoolPrices(tokens, prices, stakeToken);
  ...
  const [periodFinish, rewardRate, balance, earned_] =
    await App.ethcallProvider.all(calls);
  const weeklyRewards =
...
export async function printSynthetixPool(
  App: any,
  info: any,
  chain: any = "eth",
  ...
  _print(
    `You are staking ${info.userStaked.toFixed(6)} ${info.stakeTokenTicker} ` +
      `$${formatMoney(userStakedUsd)} (${userStakedPct.toFixed(
        2
      )}% of the pool).`
  );
  if (info.userStaked > 0) {
    ...
    _print(
      `Estimated ${info.rewardTokenTicker} earnings:` +
        ` Day ${userDailyRewards.toFixed(2)} ($${formatMoney(
  ...
  switch (chain) {
    ...
    case "avax":
      _print(
        `<a target="_blank" href="https://cchain.explorer.avax.network/address/${info.stakingAddress}#code">Explorer</a>`
      );
      break;
    case "bsc":
      _print(
        `<a target="_blank" href="https://bscscan.com/address/${info.stakingAddress}#code">BSC Scan</a>`
      );
      break;
    case "heco":
      _print(
        `<a target="_blank" href="https://hecoinfo.com/address/${info.stakingAddress}#code">Heco Scan</a>`
      );
      break;
    case "matic":
      _print(
        `<a target="_blank" href="https://explorer-mainnet.maticvigil.com/address/${info.stakingAddress}#code">Polygon Explorer</a>`
      );
      break;
    case "okex":
      _print(
        `<a target="_blank" href="https://www.oklink.com/okexchain/address/${info.stakingAddress}#code">Okex Explorer</a>`
      );
      break;
    case "kcc":
      _print(
        `<a target="_blank" href="https://explorer.kcc.io/en/address/${info.stakingAddress}#code">KUCOIN Explorer</a>`
      );
      break;
    case "fantom":
      _print(
        `<a target="_blank" href="https://ftmscan.com/address/${info.stakingAddress}#code">FTM Scan</a>`
      );
      break;
    case "fuse":
      _print(
        `<a target="_blank" href="https://explorer.fuse.io/address/${info.stakingAddress}#code">FUSE Scan</a>`
      );
      break;
    case "xdai":
      _print(
        `<a target="_blank" href="https://blockscout.com/xdai/mainnet/address/${info.stakingAddress}/contracts">Explorer</a>`
      );
      break;
  }
  if (info.stakeTokenTicker != "ETH") {
    _print_link(
      `Stake ${info.userUnstaked.toFixed(6)} ${info.stakeTokenTicker}`,
      approveTENDAndStake
    );
  } else {
  ...
  _print_link(
    `Unstake ${info.userStaked.toFixed(6)} ${info.stakeTokenTicker}`,
    unstake
  );
  _print_link(
...
export async function loadSynthetixPool(
  App: any,
  tokens: any,
  prices: any,
  abi: any,
  ...
  const info = await loadSynthetixPoolInfo(
    App,
    tokens,
    prices,
    abi,
...
export async function loadMultipleSynthetixPools(
  ...
  let totalAPR =
    totalUserStaked === 0
      ? 0
      : individualAPRs.reduce((x: any, y: any) => x + y, 0) / totalUserStaked;
  return { staked_tvl: totalStaked, totalUserStaked, totalAPR };
}

export async function loadBasisFork(data: any) {
  ...
  let p1 = await loadSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
  ...
  if (data.SharePool2) {
    let p3 = await loadSynthetixPool(
      App,
      tokens,
      prices,
      data.PoolABI,
  ...
  let p2 = await loadSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
  ...
  if (data.SeedBanks) {
    let p = await loadMultipleSynthetixPools(
      App,
      tokens,
      prices,
      data.SeedBanks
...
export async function getNewPricesAndTokens(
  App: any,
  tokens: any,
  prices: any,
  newAddresses: any,
  ...
  var newPriceAddresses = newAddresses.filter(
    (x: any) => !getParameterCaseInsensitive(prices, x)
  );
  var newPrices = await lookUpTokenPrices(newPriceAddresses);
  for (const key in newPrices) {
    if (newPrices[key]) prices[key] = newPrices[key];
  }
  var newTokenAddresses = newAddresses.filter(
    (x: any) => !getParameterCaseInsensitive(tokens, x)
  );
  for (const address of newTokenAddresses) {
    tokens[address] = await getToken(App, address, stakingAddress);
...
export async function getAverageBlockTime(App: any) {
...
export const displayPrice = (price: any) => {
  const priceDecimals =
    price === 0 ? 2 : price < 0.0001 ? 10 : price < 0.01 ? 6 : 2;
  return priceDecimals === 2
    ? formatMoney(price)
