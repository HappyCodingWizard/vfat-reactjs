...
export async function main() {
  ...
  const PolyZeus_CHEF_ADDR = "0x9DD398dfD94d40699eCb020864e3501d8a978C71";
  const rewardTokenTicker = "PolyZeus";
  const PolyZeus_CHEF = new ethers.Contract(PolyZeus_CHEF_ADDR, PolyZeus_CHEF_ABI, App.provider);

  const startBlock = await PolyZeus_CHEF.startBlock();
  const currentBlock = await App.provider.getBlockNumber();

  const multiplier = await PolyZeus_CHEF.getMultiplier(currentBlock, currentBlock + 1);
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = await PolyZeus_CHEF.zeusPerBlock() / 1e18
  ...
  await loadMaticChefContract(App, tokens, prices, PolyZeus_CHEF, PolyZeus_CHEF_ADDR, PolyZeus_CHEF_ABI, rewardTokenTicker,
    "zeus", null, rewardsPerWeek, "pendingEgg", [14]);
