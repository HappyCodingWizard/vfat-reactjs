...
export async function main() {
   ...
   const BSC_ICE_CHEF_ADDR2 = "0xbf513aCe2AbDc69D38eE847EFFDaa1901808c31c";
   ...
   const BSC_ICE_CHEF2 = new ethers.Contract(BSC_ICE_CHEF_ADDR2, BSC_ICE_CHEF2_ABI, App.provider);
   ...
   const rewardsPerWeek2 = await BSC_ICE_CHEF2.icePerSecond() /1e18 * 604800;

    const tokens = {};
    const prices = await getBscPrices();

    let p2 = await loadBscPopsicleContract(App, tokens, prices, BSC_ICE_CHEF2, BSC_ICE_CHEF_ADDR2, BSC_ICE_CHEF2_ABI, rewardTokenTicker,
      "ice", null, rewardsPerWeek2, "pendingIce");
      
    _print_bold("The following farms have been DISCONTINUED. You can unstake at any time.\n")
    
    let p0 = await loadBscPopsicleContract(App, tokens, prices, BSC_ICE_CHEF0, BSC_ICE_CHEF_ADDR0, BSC_ICE_CHEF_ABI, rewardTokenTicker,
      "ice", null, rewardsPerWeek0, "pendingIce");
    let p1 = await loadBscPopsicleContract(App, tokens, prices, BSC_ICE_CHEF1, BSC_ICE_CHEF_ADDR1, BSC_ICE_CHEF_ABI, rewardTokenTicker,
...
async function loadBscPopsicleContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBscPopsiclePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
