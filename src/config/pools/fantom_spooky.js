...
export async function main() {
  ...
  const BOO_CHEF_ADDR = "0x2b2929E785374c651a81A63878Ab22742656DcDd";
  const SINGLE_BOO_CHEF_ADDR = "0xACACa07e398d4946AD12232F40f255230e73Ca72";

  const rewardTokenTicker = "BOO";
  const BOO_CHEF = new ethers.Contract(BOO_CHEF_ADDR, BOO_CHEF_ABI, App.provider);
  const SINGLE_BOO_CHEF = new ethers.Contract(SINGLE_BOO_CHEF_ADDR, SINGLE_BOO_CHEF_ABI, App.provider);

  const rewardsPerWeek = await BOO_CHEF.booPerSecond() / 1e18 * 604800;
  const singleRewardsPerWeek = await SINGLE_BOO_CHEF.booPerSecond() / 1e18 * 604800;
...
async function loadSingleSpookyContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getSpookyPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
