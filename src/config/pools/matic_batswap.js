...
export async function main() {
    ...
    const BATSWAP_CHEF_ADDR = '0xA7439172dE1343A2F640C36eCfb576706283504B'
    const rewardTokenTicker = "BATSWAP";
    const BATSWAP_CHEF = new ethers.Contract(BATSWAP_CHEF_ADDR, BATSWAP_CHEF_ABI, App.provider);

    const startBlock = await BATSWAP_CHEF.startBlock();
    ...
    if (currentBlock < startBlock) {
        ...
        rewardsPerWeek = await BATSWAP_CHEF.batswapPerBlock() / 1e18 *
            604800 / 2.1;
    }
    ...
    await loadMaticChefContract(App, tokens, prices, BATSWAP_CHEF, BATSWAP_CHEF_ADDR, BATSWAP_CHEF_ABI, rewardTokenTicker,
        "batswap", null, rewardsPerWeek, "pendingBatSwap");
