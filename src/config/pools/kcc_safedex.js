...
export async function main() {
    ...
    const SAFEDEX_CHEF_ADDR = "0xeb1797efb1c48ab925d62cf4fc777d547dc06bd6";
    const rewardTokenTicker = "TREATS";
    const SAFEDEX_CHEF = new ethers.Contract(SAFEDEX_CHEF_ADDR, SAFEDEX_CHEF_ABI, App.provider);

    const startBlock = await SAFEDEX_CHEF.startBlock();
    const currentBlock = await App.provider.getBlockNumber();

    const multiplier = await SAFEDEX_CHEF.getMultiplier(currentBlock, currentBlock + 1);

    let rewardsPerWeek = 0
    if (currentBlock < startBlock) {
        _print(`Rewards start at block ${startBlock}\n`);
    } else {
        rewardsPerWeek = await SAFEDEX_CHEF.eggPerBlock() / 1e18
            * 604800 * multiplier / 3;
    }
