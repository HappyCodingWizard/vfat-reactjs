...
const MOUNTAIN_CHEF_ABI = [
  {
    inputs: [
      {
        internalType: 'contract MountainSwapToken',
        name: '_mountain',
        type: 'address',
      },
  ...
  },
  {
    ...
    name: 'mountain',
    outputs: [
      { internalType: 'contract MountainSwapToken', name: '', type: 'address' },
    ],
    stateMutability: 'view',
    type: 'function',
  },
  ...
  },
  {
    ...
    outputs: [
      ...
      { internalType: 'uint256', name: 'accMountainPerShare', type: 'uint256' },
  ...
  {
    inputs: [
      { internalType: 'uint256', name: '_mountainPerBlock', type: 'uint256' },
    ],
    name: 'updateEmissionRate',
    outputs: [],
    stateMutability: 'nonpayable',
...
export async function main() {
  ...
  const MOUNTAIN_CHEF_ADDR = '0x910bE5640acb136Af65fa2440C41D7d24967AC60'
  const rewardTokenTicker = "MOUNTAIN";
  const MOUNTAIN_CHEF = new ethers.Contract(MOUNTAIN_CHEF_ADDR, MOUNTAIN_CHEF_ABI, App.provider);

  const startBlock = await MOUNTAIN_CHEF.startBlock();
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = await MOUNTAIN_CHEF.mountainPerBlock() / 1e18 *
      604800 / 2.1;
  }
  ...
  await loadMaticChefContract(App, tokens, prices, MOUNTAIN_CHEF, MOUNTAIN_CHEF_ADDR, MOUNTAIN_CHEF_ABI, rewardTokenTicker,
    "mountain", null, rewardsPerWeek, "pendingMountain");
