...
export async function main() {
   ...
   const pSWAMP_CHEF_ADDR = "0x4F04e540A51013aFb6761ee73D71d2fB1F29af80";
   const rewardTokenTicker = "pSWAMP";
   const pSWAMP_CHEF = new ethers.Contract(pSWAMP_CHEF_ADDR, pSWAMP_CHEF_ABI, App.provider);

   const rewardsPerWeek = await pSWAMP_CHEF.NATIVEPerBlock() /1e18
        * 604800 / 3;

    const tokens = {};
    const prices = await getMaticPrices();

    await loadMaticpSWAMPContract(App, tokens, prices, pSWAMP_CHEF, pSWAMP_CHEF_ADDR, pSWAMP_CHEF_ABI, rewardTokenTicker,
        "NATIVE", null, rewardsPerWeek, "pendingNATIVE", [1]);

    hideLoading();
  }

  async function loadMaticpSWAMPContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
    ...
    const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
      await getMaticpSWAMPPoolInfo(App, chefContract, x, pendingRewardsFunction)));
  ...
  async function getMaticpSWAMPPoolInfo(App, chefContract, poolIndex, pendingRewardsFunction) {
...
function printpSWAMPPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
  ...
  printpSWAMPContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printpSWAMPContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
  ...
  const unstake = async function() {
    return chefpSWAMPContract_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
  }
...
const chefpSWAMPContract_unstake = async function(chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
