...
import {
  ...
  getBscToken,
    ...
    export async function main() {
      ...
      const tokens = {};
      const poolInfos = await Promise.all(Address.map(a => loadBadgerPoolInfo(App, tokens, prices, a)));
      let tvl = 0, userTvl = 0
      for(const p of poolInfos.filter(p => p))
      {
        printBadgerContract(p);
    ...
    async function loadBadgerPoolInfo(App, tokens, prices, contractAddress) {
      try {
        const contract = await new ethers.Contract(contractAddress, BADGER_VAULT_ABI, App.provider);
    ...
    async function printBadgerContract(poolInfo) {
