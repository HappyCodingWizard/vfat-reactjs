...
export async function main() {
  ...
  const OLIVE_CHEF_ADDR = "0x5A9710f3f23053573301C2aB5024D0a43A461E80";
  const rewardTokenTicker = "OLIVE";
  const OLIVE_CHEF = new ethers.Contract(OLIVE_CHEF_ADDR, OLIVE_CHEF_ABI, App.provider);

  const blocksPerSeconds = await getAverageBlockTime(App);

  const rewardsPerWeek = await OLIVE_CHEF.olivePerBlock() / 1e18
  ...
  await loadOliveChefContract(App, tokens, prices, OLIVE_CHEF, OLIVE_CHEF_ADDR, OLIVE_CHEF_ABI, rewardTokenTicker,
    "olive", null, rewardsPerWeek, "pendingOlive", [8]);

  hideLoading();
}

async function loadOliveChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
        ...
        pendingRewardsFunction, 6, null, "avax")
      aprs.push(apr);
    }
  }
