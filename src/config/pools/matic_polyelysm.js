...
export async function main() {
   ...
   const ELYSM_CHEF_ADDR = "0xe681c22Dc729E88559a0607ACa4b136Cc9998A6F";
   const rewardTokenTicker = "ELYSM";
   const ELYSM_CHEF = new ethers.Contract(ELYSM_CHEF_ADDR, ELYSM_CHEF_ABI, App.provider);

   const startBlock = await ELYSM_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await ELYSM_CHEF.getMultiplier(currentBlock, currentBlock + 1);
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await ELYSM_CHEF.ELYSMPerBlock() /1e18
    ...
    await loadMaticChefContract(App, tokens, prices, ELYSM_CHEF, ELYSM_CHEF_ADDR, ELYSM_CHEF_ABI, rewardTokenTicker,
        "ELYSM", null, rewardsPerWeek, "pendingELYSM");
