...
export async function main() {
  ...
  const HONEY_CHEF_ADDR = "0x7fD7E3C7F7dED31157A5CC62132569020e699C5F";
  const rewardTokenTicker = "yBHONEY";
  const HONEY_CHEF = new ethers.Contract(HONEY_CHEF_ADDR, HONEY_CHEF_ABI, App.provider);

  const rewardsPerWeek = await HONEY_CHEF.rewardPerBlock() / 1e18
    * 604800 / 3;

  const tokens = {};
  const prices = await getBscPrices();

  await loadBscBearChefContract(App, tokens, prices, HONEY_CHEF, HONEY_CHEF_ADDR, HONEY_CHEF_ABI, rewardTokenTicker,
    "erc20", null, rewardsPerWeek, "pending");

  hideLoading();
}

async function loadBscBearChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = prinBeartChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
function prinBeartChefPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
  totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
