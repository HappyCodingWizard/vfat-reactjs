...
export async function main() {
  ...
  let p = await loadSandboxPool(App, tokens, prices, Pool.abi, Pool.address, Pool.rewardTokenAddress, Pool.stakeTokenAddress)
...
async function loadSandboxPool(App, tokens, prices, abi, address, rewardTokenAddress, stakeTokenAddress) {
  const info = await loadSandboxPoolInfo(App, tokens, prices, abi, address, rewardTokenAddress, stakeTokenAddress);
  return await printSynthetixPool(App, info);
}

async function loadSandboxPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  tokenAddress, stakingTokenAddress) {
  const STAKING_POOL = new ethers.Contract(stakingAddress, stakingAbi, App.provider);
  const STAKING_MULTI = new ethcall.Contract(stakingAddress, stakingAbi);

  const stakeTokenAddress = stakingTokenAddress;
