...
export async function lp_composition(pool, token0Balance, token1Balance) {
  _print(`Your LP tokens comprise of ${token0Balance.toFixed(8)} ${pool.token0} + ${token1Balance.toFixed(8)} ${pool.token1}`)
}

export async function loadLiquidityMiningInfo(App, pool, blockTime, KyberRewardLocker, TOKEN_ADDRESSES, POOLS, REWARDS) {
...
export async function printLiquidityMiningInfo(App, info, blockTime, explorer, TOKEN_ADDRESSES, REWARDS) {
  _print(
  ...
  const claim = async function () {
    ...
    const KyberRewardLocker = '0xfab5186A194588F5AD5074Bd52659302906B4522' // test
