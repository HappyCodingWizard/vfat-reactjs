...
export async function main() {
  ...
  const PMP_CHEF_ADDR = "0x29142471a5c33a2a4cD7C8f18Ce881F699b0c681";
  const rewardTokenTicker = "PMP";
  const PMP_CHEF = new ethers.Contract(PMP_CHEF_ADDR, PMP_CHEF_ABI, App.provider);

  const rewardsPerWeek = await PMP_CHEF.PMPPerBlock() / 1e18
    * 604800 / 3;

  const tokens = {};
  const prices = await getBscPrices();

  await loadBscPumpyContract(App, tokens, prices, PMP_CHEF, PMP_CHEF_ADDR, PMP_CHEF_ABI, rewardTokenTicker,
    "PMPTokenAddress", null, rewardsPerWeek, "pendingPMP");

  hideLoading();
}

async function loadBscPumpyContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBscPumpyPoolInfo(App, chefContract, x, pendingRewardsFunction)));
...
async function getBscPumpyPoolInfo(App, chefContract, poolIndex, pendingRewardsFunction) {
  const poolInfo = await chefContract.poolInfo(poolIndex);
  if (poolInfo.allocPoint === 0 || poolInfo.accPMPPerShare === 0) {
    return {
