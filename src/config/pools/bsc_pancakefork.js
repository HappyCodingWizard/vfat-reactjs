...
export async function main() {
  ...
  const FCKE_CHEF_ADDR = "0x0F2509FaD69DC74f3DACB6eB854628ec1dffFd1c";
  const rewardTokenTicker = "CAKF";
  const FCKE_CHEF = new ethers.Contract(FCKE_CHEF_ADDR, FCKE_CHEF_ABI, App.provider);

  const farmRewardsPerWeek = await FCKE_CHEF.cakfFarmRewardsPerBlock() / 1e18
    * 604800 / 3;
  ...
  await loadFarmFCKEContract(App, tokens, prices, FCKE_CHEF, FCKE_CHEF_ADDR, FCKE_CHEF_ABI, rewardTokenTicker,
    "cakf", null, farmRewardsPerWeek, "pendingCAKF", [1]);
...
async function getPoolFCKEInfo(App, chefContract, chefAddress, poolIndex) {
  ...
  const poolInfo = await poolInfoContract.poolInfo();
