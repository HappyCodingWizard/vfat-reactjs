...
export async function main() {
  ...
  const BLZD_CHEF_ADDR = "0x367CdDA266ADa588d380C7B970244434e4Dde790";
  const rewardTokenTicker = "xBLZD";
  const BLZD_CHEF = new ethers.Contract(BLZD_CHEF_ADDR, BLZD_CHEF_ABI, App.provider);

  const rewardsPerWeek = await BLZD_CHEF.xBLZDPerBlock() / 1e18
    * 604800 / 3;

  const tokens = {};
  const prices = await getBscPrices();

  await loadBscChefContract(App, tokens, prices, BLZD_CHEF, BLZD_CHEF_ADDR, BLZD_CHEF_ABI, rewardTokenTicker,
    "xBLZD", null, rewardsPerWeek, "pendingxBLZD", [1]);
...
export async function loadBscChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBlizzardPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
...
async function getBlizzardPoolInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
