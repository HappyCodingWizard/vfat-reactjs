...
export async function main() {
  ...
  const SHARK_CHEF_ADDR = "0x1414529422AdD5a715C3dE929C32B2d88aE4A761";
  const rewardTokenTicker = "SHARK";
  const SHARK_CHEF = new ethers.Contract(SHARK_CHEF_ADDR, SHARK_CHEF_ABI, App.provider);

  const startBlock = await SHARK_CHEF.startBlock();
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = await SHARK_CHEF.SharkPerBlock() / 1e18
      * 604800 / 2.1;
  }
  ...
  await loadSharkContract(App, tokens, prices, SHARK_CHEF, SHARK_CHEF_ADDR, SHARK_CHEF_ABI, rewardTokenTicker,
    "Shark", null, rewardsPerWeek, "pendingShark");

  hideLoading();
}

async function loadSharkContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getSharkPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printSharkPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
        totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
        pendingRewardsFunction, poolInfos[i].fee, null, null, "matic")
      aprs.push(apr);
    }
  }
...
async function getSharkPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
function printSharkPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
  totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printSharkContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printSharkContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
