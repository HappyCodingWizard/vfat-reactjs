...
export async function getFusePrices() {
...
export async function getFuseUniPool(App, pool, poolAddress, stakingAddress) {
  let q0, q1;
...
export async function getFuseErc20(App, token, address, stakingAddress) {
  if (address === "0x0000000000000000000000000000000000000000") {
    ...
    return {
      ...
      totalSupply: await App.provider.getBalance(address),
...
export async function getFuseVault(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
  const token_ = await vault.token();
  const token = await getFuseToken(App, token_, address);
  return {
    address,
...
export async function getFuseWantVault(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
...
export async function getFuseStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    case "uniswap":
      const pool = new ethers.Contract(tokenAddress, UNI_ABI, App.provider);
      return await getFuseUniPool(App, pool, tokenAddress, stakingAddress);
    case "fuseVault":
      const vault = new ethers.Contract(tokenAddress, FUSE_VAULT_TOKEN_ABI, App.provider);
      return await getFuseVault(App, vault, tokenAddress, stakingAddress);
    case "fuseWantVault":
      const wantVault = new ethers.Contract(tokenAddress, FUSE_VAULT_WANT_ABI, App.provider);
...
export async function getFuseToken(App, tokenAddress, stakingAddress) {
...
export async function loadFuseSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  ...
  var stakeToken = await getFuseToken(App, stakeTokenAddress, stakingAddress);
...
export async function loadFuseSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFuseSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printSynthetixPool(App, info, "fuse");
}

export async function loadFuseBasisFork(data) {
...
export async function getFusePoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  const poolToken = await getFuseToken(app, poolInfo.lpToken ?? poolInfo.token, chefAddress);
...
export async function loadFuseChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getFusePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
        ...
        pendingRewardsFunction, null, null, "fuse", poolInfos[i].depositFee, poolInfos[i].withdrawFee)
      aprs.push(apr);
    }
  }
...
export async function loadMultipleFuseSynthetixPools(App, tokens, prices, pools) {
  let totalStaked = 0, totalUserStaked = 0, individualAPRs = [];
  const infos = await Promise.all(pools.map(p =>
    loadFuseSynthetixPoolInfo(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction)));
...
export async function loadMultipleFuseSynthetixPoolsSequential(App, tokens, prices, pools) {
  let totalStaked = 0, totalUserStaked = 0, individualAPRs = [];
  for (const p of pools) {
    let res = await loadFuseSynthetixPool(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction);
    if (!res) continue;
