...
const LOOT_PIT_ADDR = "0xA15C7828Ab22D182383A84F828Cd71Ac09Bb55E8";
const REWARD_TOKEN_TICKER = "LOOT";
const PIT_NAME = "AutoLooter";
...
export async function main() {
  ...
  const LOOT_CHEF = new ethers.Contract(LOOT_CHEF_ADDR, LOOT_CHEF_ABI, App.provider);

  const blockNumber = await App.provider.getBlockNumber();
  const multiplier = await LOOT_CHEF.getMultiplier(blockNumber, blockNumber + 1);
  const rewardPerBlock = await LOOT_CHEF.REWARD_PER_BLOCK();
  ...
  const LOOT = new ethers.Contract(LOOT_TOKEN_ADDR, ERC20_ABI, App.provider.getSigner());
  const LOOT_PIT = new ethers.Contract(LOOT_PIT_ADDR, LOOT_PIT_ABI, App.provider.getSigner());
  const lootPit = await lootPitData(LOOT, LOOT_PIT, App, prices);

  const totalStakedInclLootPit = totalStaked + lootPit.totalBalanceUSD

  _print_bold(`Total Staked: $${formatMoney(totalStakedInclLootPit)}`);
  ...
  if (lootPit.userBalanceUSD > 0) {
    _print(`You are staking a total of $${formatMoney(lootPit.userBalanceUSD)} in the ${PIT_NAME} single staking pool.`);
  }

  hideLoading();
}

async function lootPitData(LOOT, LOOT_PIT, App, prices) {
  const lootPitTotalBalance = await LOOT.balanceOf(LOOT_PIT_ADDR) / 1e18;
  const lootPitTotalSupply = await LOOT_PIT.totalSupply() / 1e18;
  const aLootRatio = lootPitTotalBalance / lootPitTotalSupply;

  const lootPitUserBalanceBigNum = await LOOT_PIT.balanceOf(App.YOUR_ADDRESS);
  const lootPitUserBalance = (lootPitUserBalanceBigNum / 1e18) * aLootRatio;
  const lootPitShare = lootPitUserBalance / lootPitTotalBalance;
  const lootBalanceBigNum = await LOOT.balanceOf(App.YOUR_ADDRESS);
  const lootBalance = lootBalanceBigNum / 1e18;

  const lootPrice = prices[LOOT_TOKEN_ADDR];

  let lootPitTotalBalanceUSD;
  let lootPitUserBalanceUSD;

  if (lootPrice && lootPrice.usd) {
    lootPitTotalBalanceUSD = lootPitTotalBalance * lootPrice.usd;
    lootPitUserBalanceUSD = lootPitUserBalance * lootPrice.usd;

    _print(`${PIT_NAME} TVL: $${formatMoney(lootPitTotalBalanceUSD)}`);
    _print(`${REWARD_TOKEN_TICKER} Price: $${formatMoney(lootPrice.usd)}`);
    _print(`Staked: ${(lootPitTotalBalance).toFixed(2)} ${REWARD_TOKEN_TICKER} ($${formatMoney(lootPitTotalBalanceUSD)})`);
    _print(`You are staking ${(lootPitUserBalance).toFixed(2)} ${REWARD_TOKEN_TICKER} ($${formatMoney(lootPitUserBalanceUSD)}), ${(lootPitShare * 100).toFixed(2)}% of the pool.`);

    const approveAndEnter = async function () {
      return lootPitApproveAndEnter(LOOT, LOOT_PIT, lootBalanceBigNum, App);
    }

    const leave = async function () {
      return lootPitLeave(LOOT_PIT, lootPitUserBalanceBigNum, App);
    }

    _print_link(`Stake ${(lootBalance).toFixed(2)} ${REWARD_TOKEN_TICKER}`, approveAndEnter);

    if (lootPitUserBalance > 0) {
      _print_link(`Unstake ${lootPitUserBalance.toFixed(2)} ${REWARD_TOKEN_TICKER}`, leave);
    }
  ...
  return {
    totalBalance: lootPitTotalBalance,
    totalBalanceUSD: lootPitTotalBalanceUSD,
    userBalance: lootPitUserBalance,
    userBalanceUSD: lootPitUserBalanceUSD
  }
}

async function lootPitApproveAndEnter(LOOT, LOOT_PIT, currentTokens, App) {
  const allowedTokens = await LOOT.allowance(App.YOUR_ADDRESS, LOOT_PIT_ADDR)
  let allow = Promise.resolve()

  if (allowedTokens / 1e18 < currentTokens / 1e18) {
    showLoading()
    allow = LOOT.approve(LOOT_PIT_ADDR, ethers.constants.MaxUint256)
      .then(function (t) {
  ...
  if (currentTokens / 1e18 > 0) {
    ...
    allow
      .then(async function () {
        LOOT_PIT.enter(currentTokens, { gasLimit: 500000 })
          .then(function (t) {
...
async function lootPitLeave(LOOT_PIT, currentStakedAmount, App) {
  showLoading()
  LOOT_PIT.leave(currentStakedAmount, { gasLimit: 500000 })
    .then(function (t) {
      App.provider.waitForTransaction(t.hash).then(function () {
        hideLoading()
      })
