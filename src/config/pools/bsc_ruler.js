...
import {
  ...
  formatMoney,
  rewardsContract_claim,
  rewardsContract_unstake,
  rewardsContract_exit,
  rewardsContract_resetApprove,
  rewardsContract_stake,
  _print_link,
  _print_bold,
} from "./ethers_helper";
import {
...
export async function main() {
  ...
  for (let poolAddress of poolAddresses) {
    ...
    if (rewards.length > 0) {
      pools.push({
        abi: RULER_CONTRACT_ABI,
        address: RULER_CONTRACT_ADDR,
  ...
  let p = await loadRulerprotocolContracts(App, tokens, prices, pools, RULER_CONTRACT)
...
async function loadRulerprotocolContracts(App, tokens, prices, pools, rulerContract) {
  let totalStaked = 0, totalUserStaked = 0, individualAPRs = [];
  const infos = await Promise.all(pools.map(p =>
    loadRulerprotocolPoolInfo(App, tokens, p.address, p.stakedToken, p.rewards, rulerContract)));
  let tokenAddresses = []
  ...
  for (const info of infos) {
    let p = await printRulerSynthetixPool(App, tokens, prices, info);
...
async function loadRulerprotocolPoolInfo(App, tokens, stakingAddress, stakeTokenAddress,
  rewards, rulerContract) {
  const rewardTokens = await Promise.all(rewards.map(async r => {
    if (!getParameterCaseInsensitive(tokens, r.rewardToken)) {
      tokens[r.rewardToken] = await getBscToken(App, r.rewardToken, stakingAddress);
    }
  ...
  const stakeToken = await getBscToken(App, stakeTokenAddress, stakingAddress);

  const userInfos = await rulerContract.getUser(stakeTokenAddress, App.YOUR_ADDRESS);
...
async function printRulerSynthetixPool(App, tokens, prices, info, chain = "eth") {
