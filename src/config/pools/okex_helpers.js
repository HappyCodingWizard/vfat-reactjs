...
async function getOkexUniPool(App, pool, poolAddress, stakingAddress) {
  let q0, q1;
...
async function getOkexVault(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
  const token_ = await vault.token();
  const token = await getOkexToken(App, token_, address);
  return {
    address,
...
async function getOkexWantVault(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
...
async function getOkexStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    case "uniswap":
      const pool = new ethers.Contract(tokenAddress, UNI_ABI, App.provider);
      return await getOkexUniPool(App, pool, tokenAddress, stakingAddress);
    case "okexVault":
      const vault = new ethers.Contract(tokenAddress, OKEX_VAULT_TOKEN_ABI, App.provider);
      return await getOkexVault(App, vault, tokenAddress, stakingAddress);
    case "okexWantVault":
      const wantVault = new ethers.Contract(tokenAddress, OKEX_VAULT_WANT_ABI, App.provider);
      return await getOkexWantVault(App, wantVault, tokenAddress, stakingAddress);
    case "erc20":
...
async function getOkexToken(App, tokenAddress, stakingAddress) {
  if (tokenAddress === "0x0000000000000000000000000000000000000000") {
    return geterc20(App, null, tokenAddress, "")
  }
  const type = window.localStorage.getItem(tokenAddress);
  if (type) return getOkexStoredToken(App, tokenAddress, stakingAddress, type);
  try {
    const pool = new ethers.Contract(tokenAddress, UNI_ABI, App.provider);
    const _token0 = await pool.token0();
    const uniPool = await getOkexUniPool(App, pool, tokenAddress, stakingAddress);
  ...
  try {
    ...
    const vault = await getOkexVault(App, VAULT, tokenAddress, stakingAddress);
    window.localStorage.setItem(tokenAddress, "okexVault");
    return vault;
  }
  catch (err) {
  }
  try {
    const WANT_VAULT = new ethers.Contract(tokenAddress, OKEX_VAULT_WANT_ABI, App.provider);
    const _want = await WANT_VAULT.want();
    const wantVault = await getOkexWantVault(App, WANT_VAULT, tokenAddress, stakingAddress);
    window.localStorage.setItem(tokenAddress, "okexWantVault");
    return wantVault;
  }
  catch (err) {
  }
...
async function loadOkexSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
export async function loadOkexSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadOkexSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printSynthetixPool(App, info, "okex");
}

export async function loadOkexBasisFork(data) {
...
export async function getOkexPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
export async function loadOkexChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
...
export async function loadSteakChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  rewardTokenFunction, rewardsPerBlockFunction, rewardsPerWeekFixed, pendingRewardsFunction,
  deathPoolIndices, claimFunction) {
  const chefContract = chef ?? new ethers.Contract(chefAddress, chefAbi, App.provider);
...
async function loadMultipleOkexSynthetixPools(App, tokens, prices, pools) {
  ...
  const infos = await Promise.all(pools.map(p =>
    loadOkexSynthetixPoolInfo(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction)));
...
export async function loadMultipleOkexSynthetixPoolsSequential(App, tokens, prices, pools) {
  let totalStaked = 0, totalUserStaked = 0, individualAPRs = [];
  for (const p of pools) {
    let res = await loadOkexSynthetixPool(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction);
    if (!res) continue;
