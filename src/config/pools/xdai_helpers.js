...
export async function getXdaiPrices() {
...
async function getXdai20(App, token, address, stakingAddress) {
...
async function getXdaiStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    ...
    case "xdai20":
      ...
      return await getXdai20(App, xdai20, tokenAddress, stakingAddress);
    default:
      break;
  }
}

export async function getXdaiToken(App, tokenAddress, stakingAddress) {
  if (tokenAddress === "0x0000000000000000000000000000000000000000") {
    return getXdai20(App, null, tokenAddress, "")
  }
...
export async function loadXdaiSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadXdaiSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printSynthetixPool(App, info, "xdai");
}

export async function loadXdaiBasisFork(data) {
  ...
  if (data.SeedBanks) {
    let p = await loadMultipleXdaiSynthetixPools(App, tokens, prices, data.SeedBanks)
...
async function getXdaiPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  const poolToken = await getXdaiToken(app, poolInfo.lpToken, chefAddress);
...
export async function loadXdaiChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getXdaiPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
...
export async function loadMultipleXdaiSynthetixPoolsSequential(App, tokens, prices, pools) {
  let totalStaked = 0, totalUserStaked = 0, individualAPRs = [];
  for (const p of pools) {
    let res = await loadXdaiSynthetixPool(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction);
    if (!res) continue;
