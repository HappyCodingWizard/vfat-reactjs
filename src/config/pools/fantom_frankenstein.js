...
export async function main() {
  ...
  const FRANK_CHEF_ADDR = "0xd920cae9EF3BA9DE82BcdB93015A90b0A81F32E3";
  const rewardTokenTicker = "FRANK";
  const FRANK_CHEF = new ethers.Contract(FRANK_CHEF_ADDR, FRANK_FTM_CHEF_ABI, App.provider);

  const rewardsPerWeek = await FRANK_CHEF.NATIVEPerBlock() / 1e18
    * 604800 / 3;

  const tokens = {};
  const prices = await getFantomPrices();

  await loadFantomFRANKContract(App, tokens, prices, FRANK_CHEF, FRANK_CHEF_ADDR, FRANK_FTM_CHEF_ABI, rewardTokenTicker,
    "NATIVE", null, rewardsPerWeek, "pendingNATIVE", [1]);

  hideLoading();
}

async function loadFantomFRANKContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getFantomFRANKPoolInfo(App, chefContract, x, pendingRewardsFunction)));
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printFRANKPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getFantomFRANKPoolInfo(App, chefContract, poolIndex, pendingRewardsFunction) {
...
function printFRANKPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
  totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printFRANKContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printFRANKContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
  ...
  const unstake = async function () {
    return chefFRANKContract_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
  }
...
const chefFRANKContract_unstake = async function (chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
