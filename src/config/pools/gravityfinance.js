...
async function loadFarmOtherDetails(App, farmAddress, aTokenAddress, bTokenAddress, ERC20ABI, FARMABI, Bzereos, prices, GFIprice) {
    ...
    if (otherSymbol === 'LINK') {
        dexguruTokenlink = `<a href='https://dex.guru/token/0x53e0bca35ec356bd5dddfebbd1fc0fd03fabad39-polygon' rel='noopener' target='_blank'>[%]</a>`;
    }
    if (otherSymbol === "SUSHI") {
        dexguruTokenlink = `<a href='https://dex.guru/token/0x0b3f868e0be5597d5db7feb59e1cadbb0fdda50a-polygon' rel='noopener' target='_blank'>[%]</a>`;
    }
...
async function get_pool_APR(App, GFI_in_pool, farm_info, ERC20ABI, farm_address) {
    ...
    if (blockNumber <= 14998366) {
        APR = blockReward * blocks_per_year / farm_GFI_worth * 100 * 2; //bonus time
    } else {
        APR = blockReward * blocks_per_year / farm_GFI_worth * 100; //blockreward * blocks_per_year = total GFI rewarded in 1 year. Divide by total staked worth * 100 to get APR in %
    }
...
async function get_APR_GFI_pool(App, GFIstakedInFarm, farm_info) {
    ...
    let APR = blockReward * blocks_per_year / GFIstakedInFarm * 100;
