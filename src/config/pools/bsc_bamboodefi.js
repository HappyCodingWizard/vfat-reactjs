...
export async function main() {
    ...
    const ZOOKEEPER_ADDR = "0x89929ef8f5094c023283AdE38Ee27a6f5e7a1D6E";
...
async function getBambooPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction, showAll=false) {
    ...
    const poolToken = await getBscToken(app, poolInfo.lpToken ?? poolInfo.stakingToken, chefAddress);
...
async function printZookeeperContractPoolsTable(title, App, chef, chefAddress, chefAbi, rewardTokenTicker,
    ...
    const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) => {
        try {
            return await getBambooPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction, showAll);
        }
        catch (ex) {
