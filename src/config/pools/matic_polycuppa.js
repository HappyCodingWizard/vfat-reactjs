...
export async function main() {
    ...
    const CUPPA_CHEF_ADDR = "0x95FDc452152bd606F3c9B7d5c744a551A86b2520";

    const rewardTokenTicker = "CUPPA";

    const CUPPA_CHEF = new ethers.Contract(CUPPA_CHEF_ADDR, CUPPA_CHEF_ABI, App.provider);



    const startBlock = await CUPPA_CHEF.startBlock();

    const currentBlock = await App.provider.getBlockNumber();



    const multiplier = await CUPPA_CHEF.getMultiplier(currentBlock, currentBlock+1);
    ...
    if (currentBlock < startBlock) {
        ...
        rewardsPerWeek = await CUPPA_CHEF.CuppaPerBlock() / 1e18 *
    ...
    await loadMaticChefContract(App, tokens, prices, CUPPA_CHEF, CUPPA_CHEF_ADDR, CUPPA_CHEF_ABI, rewardTokenTicker,

        "cuppa", null, rewardsPerWeek, "pendingCuppa");
