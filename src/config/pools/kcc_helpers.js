...
export async function getKccPrices() {
...
export async function getKccUniPool(App, pool, poolAddress, stakingAddress) {
  let q0, q1;
...
async function getKccVault(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
  const token_ = await vault.token();
  const token = await getKccToken(App, token_, address);
  return {
    address,
...
async function getKccWantVault(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
...
export async function getKccStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    case "uniswap":
      const pool = new ethers.Contract(tokenAddress, UNI_ABI, App.provider);
      return await getKccUniPool(App, pool, tokenAddress, stakingAddress);
    case "kccVault":
      const vault = new ethers.Contract(tokenAddress, KCC_VAULT_TOKEN_ABI, App.provider);
      return await getKccVault(App, vault, tokenAddress, stakingAddress);
    case "kccWantVault":
      const wantVault = new ethers.Contract(tokenAddress, KCC_VAULT_WANT_ABI, App.provider);
      return await getKccWantVault(App, wantVault, tokenAddress, stakingAddress);
    case "erc20":
...
export async function getKccToken(App, tokenAddress, stakingAddress) {
...
async function loadKccSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
export async function loadKccSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadKccSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printSynthetixPool(App, info, "kcc");
}

export async function loadKccBasisFork(data) {
...
async function getKccPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  const poolToken = await getKccToken(app, poolInfo.lpToken ?? poolInfo.token ?? poolInfo.stakingToken, chefAddress);
...
export async function loadKccChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
        ...
        pendingRewardsFunction, null, claimFunction, "kcc", poolInfos[i].depositFee, poolInfos[i].withdrawFee)
      aprs.push(apr);
    }
  }
...
export async function loadMultipleKccSynthetixPoolsSequential(App, tokens, prices, pools) {
  let totalStaked = 0, totalUserStaked = 0, individualAPRs = [];
  for (const p of pools) {
    let res = await loadKccSynthetixPool(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction);
    if (!res) continue;
