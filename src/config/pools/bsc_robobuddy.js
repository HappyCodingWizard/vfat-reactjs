...
export async function main() {
   ...
   const ROBO_CHEF_ADDR = "0x844D4e12c05171aB6115A4E5730254391531840E";
   const rewardTokenTicker = "ROBO";
   const ROBO_CHEF = new ethers.Contract(ROBO_CHEF_ADDR, ROBO_CHEF_ABI, App.provider);

   const startBlock = await ROBO_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await ROBO_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await ROBO_CHEF.roboPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, ROBO_CHEF, ROBO_CHEF_ADDR, ROBO_CHEF_ABI, rewardTokenTicker,
        "robo", null, rewardsPerWeek, "pendingRobo", [8]);
