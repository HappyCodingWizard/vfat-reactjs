...
export async function main() {
  ...
  const PCAKE_CHEF_ADDR = "0xDC06a5aD9F76Fb5DfAE69Ca629ae3632E6F66994";
  const rewardTokenTicker = "PCAKE";
  const PCAKE_CHEF = new ethers.Contract(PCAKE_CHEF_ADDR, PCAKE_CHEF_ABI, App.provider);

  const startBlock = await PCAKE_CHEF.startBlock();
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = await PCAKE_CHEF.PcakePerBlock() / 1e18
      * 604800 / 2.1;
  }
  ...
  await loadMaticChefContract(App, tokens, prices, PCAKE_CHEF, PCAKE_CHEF_ADDR, PCAKE_CHEF_ABI, rewardTokenTicker,
    "Pcake", null, rewardsPerWeek, "pendingPcake");
