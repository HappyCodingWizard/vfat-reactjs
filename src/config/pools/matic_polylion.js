...
export async function main() {
  ...
  const LION_CHEF_ADDR = "0x00275072A952f7731d507dc5deC9Bcb27c13cfc3";
  const rewardTokenTicker = "LION";
  const LION_CHEF = new ethers.Contract(LION_CHEF_ADDR, LION_CHEF_ABI, App.provider);

  const startBlock = await LION_CHEF.startBlock();
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = await LION_CHEF.lionPerBlock() / 1e18
      * 604800 / 2.1;
  }
  ...
  await loadLionContract(App, tokens, prices, LION_CHEF, LION_CHEF_ADDR, LION_CHEF_ABI, rewardTokenTicker,
    "lion", null, rewardsPerWeek, "pendingLion", [8]);

  hideLoading();
}

async function loadLionContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printLionPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
function printLionPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
  totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printLionContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain, depositFee, withdrawFee);
  return apr;
}

function printLionContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
  rewardTokenTicker, stakeTokenTicker, unstaked, userStaked, pendingRewardTokens, fixedDecimals,
  claimFunction, rewardTokenPrice, chain, depositFee, withdrawFee) {
  fixedDecimals = fixedDecimals ?? 2;
  const approveAndStake = async function () {
    return lionContract_stake(chefAbi, chefAddr, poolIndex, poolAddress, App)
  }
...
const lionContract_stake = async function (chefAbi, chefAddress, poolIndex, stakeTokenAddr, App) {
