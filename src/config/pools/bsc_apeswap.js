...
export async function main() {
  ...
  const APESWAP_CHEF = new ethers.Contract(APESWAP_CHEF_ADDR, APESWAP_CHEF_ABI, App.provider);
  const APESWAP_SOUL_CHEF = new ethers.Contract(APESWAP_SOUL_CHEF_ADDR, APESWAP_SOUL_CHEF_ABI, App.provider);
  ...
  await loadSoulChefContract(App, tokens, prices, APESWAP_SOUL_CHEF, APESWAP_SOUL_CHEF_ADDR, APESWAP_SOUL_CHEF_ABI, rewardSoulTokenTicker,
    "rewardToken", null, rewardsSoulPerWeek, "pendingReward");
...
async function loadSoulChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfo = await getSoulPoolInfo(App, chefContract, chefAddress, pendingRewardsFunction);

  let tokenAddresses = poolInfo.poolToken.tokens;
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrice) {
      const apr = printChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfo, i, poolPrice,
...
async function getSoulPoolInfo(App, chefContract, chefAddress, pendingRewardsFunction) {
  const poolInfo = await chefContract.poolInfo(0);
