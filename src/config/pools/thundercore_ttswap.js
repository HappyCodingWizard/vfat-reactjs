...
import {
  ...
  TT_SWAP_FACTORY_ADDRESS,
  TTSwapFactoryABI,
  getTTUsdFromHuobi,
} from "./thundercore_helpers";
...
export async function main() {
  ...
  const TOKEN_PAIRS = INITIAL_TOKENS_CONTEXT[parseInt(NETWORKS.THUNDERCORE.chainId)]
  let tokenContracts = {}
  ...
  for (const pair of Object.keys(TOKEN_PAIRS)) {
    ...
    _print(
      ...
      ).replace(/(\.0+|0+)$/, "")} / ${formatMoney(formattedPrice) === "0.00" ? "< $0.01" : "$" + formatMoney(formattedPrice).replace(/(\.0+|0+)$/, "")
      } )`
    )
  }
